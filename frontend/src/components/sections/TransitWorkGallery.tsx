...
const FALLBACK_ASSETS: TransitAsset[] = [
  {
    id: "tvs01.png",
    filename: "tvs01.png",
    client: "TVS Motors",
...
export function TransitWorkGallery() {
  const [assets, setAssets] = useState<TransitAsset[]>(FALLBACK_ASSETS);
  const [loading, setLoading] = useState(false);
  const [activeImage, setActiveImage] = useState<TransitAsset | null>(null);

  useEffect(() => {
    const controller = new AbortController();
    fetch(`${API}/api/transit-assets`, { signal: controller.signal })
      .then((res) => (res.ok ? res.json() : []))
      .then((data) => {
        if (Array.isArray(data) && data.length > 0) setAssets(data);
      })
      .catch((err) => {
        console.warn("Could not fetch transit assets from backend:", err);
      })
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-3 sm:mt-4 text-display-md font-semibold text-fg uppercase tracking-tight">
              Outdoor & Transit Branding Work in the Wild
            </h2>
            <p className="mt-2 text-xs sm:text-sm text-muted max-w-xl">
              Original scale collage of high-visibility public bus panels, auto hood wraps, and city transit media campaigns.
            </p>
          </div>
                ...
                <div className="flex items-center justify-between border-b border-white/10 px-4 py-3 sm:px-6">
                  <div className="min-w-0 pr-4">
                    <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-brand-light block truncate">
                      {activeImage.client} — Outdoor & Transit Branding
                    </span>
